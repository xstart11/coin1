...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("DRLMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("DRLMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("DRLMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("daral-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("DRLMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running DRLMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadDRLMiner() exception");
    } catch (...) {
        LogPrintf("ThreadDRLMiner() exception");
    }

    LogPrintf("ThreadDRLMiner exiting\n");
}
